...
public class WorkspaceMemberController {
    ...
    public ResponseEntity<ApiResponse<WorkspaceMemberResponse>> inviteMember(
        ...
        WorkspaceMemberResponse response = workspaceService.inviteMember(workspaceUuid, request, userDetails.getUser());
    ...
    public ResponseEntity<ApiResponse<List<WorkspaceMemberResponse>>> getMembers(
            @Parameter(description = "워크스페이스 UUID") @PathVariable UUID workspaceUuid,
            @Parameter(hidden = true) @AuthenticationPrincipal CustomUserDetails userDetails) {
        List<WorkspaceMemberResponse> responses = workspaceService.getMembers(workspaceUuid, userDetails.getUser());
    ...
    public ResponseEntity<Void> removeMember(
            @Parameter(description = "워크스페이스 UUID") @PathVariable UUID workspaceUuid,
            @Parameter(description = "내보낼 사용자 ID (PK)") @PathVariable Long userId,
            @Parameter(hidden = true) @AuthenticationPrincipal CustomUserDetails userDetails) {
        workspaceService.removeMember(workspaceUuid, userId, userDetails.getUser());
